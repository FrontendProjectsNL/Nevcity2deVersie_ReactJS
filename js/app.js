...
const counter1 = new Counter(0, 120, ".stats__counter-one", 5000);
const counter2 = new Counter(0, 12, ".stats__counter-two", 5000);
const counter3 = new Counter(0, 98, ".stats__counter-three", 5000);

/*
  Intersection Observer About Us: 
  */

const aboutUsText = document.getElementsByClassName("about-us__text-wrapper");

const aboutUsTextObserverCallback = (
  aboutUsTextToWatch,
  aboutUsTextObserver
) => {
...
const aboutUsTextObserver = new IntersectionObserver(
  aboutUsTextObserverCallback,
  aboutUsTextObserverOPtions
);
let i = 0;
...
*/
const aboutUsImages = document.getElementsByClassName("about-us__image");

const aboutUsImagesObserverCallback = (
  aboutUsImagesToWatch,
  aboutUsImagesObserver
) => {
  /*je MOET hier een loop gebruiken: ookal heb je 1 paragraaf: */
...
const aboutUsImagesObserver = new IntersectionObserver(
  aboutUsImagesObserverCallback,
  aboutUsImagesObserverOPtions
);
i = 0;
...
const stats1 = document.getElementsByClassName("stats__card1");
...
const stats1Observer = new IntersectionObserver(
  stats1ObserverCallback,
  stats1ObserverOPtions
);
stats1Observer.observe(stats1[0]);

/* Stats2 intersection observer : */
const stats2 = document.getElementsByClassName("stats__card2");
...
const stats2Observer = new IntersectionObserver(
  stats2ObserverCallback,
  stats2ObserverOPtions
);
stats2Observer.observe(stats2[0]);

/* Stats3 intersection observer : */
const stats3 = document.getElementsByClassName("stats__card3");
...
const stats3Observer = new IntersectionObserver(
  stats3ObserverCallback,
  stats3ObserverOPtions
);
...
var Tripousel = function (options) {
  ...
  _this.slide = function (dir) {
    var inViewRight = _this.container.querySelector(
        ".carousel__panel--inView.carousel__panel--right"
      ),
      inViewCenter = _this.container.querySelector(
        ".carousel__panel--inView.carousel__panel--center"
      ),
      inViewLeft = _this.container.querySelector(
  ...
  _this.updateContent = function (e) {
    ...
    if (!e) {
      // <- for when we need this to auto-advance through items
      dir = 1;
    } else {
    ...
    switch (dir == 1) {
      case true:
        ...
        _this.center =
          _this.center < _this.items.length - 1 ? _this.center + 1 : 0;
        _this.right =
          _this.right < _this.items.length - 1 ? _this.right + 1 : 0;

        outView.style.backgroundImage =
          'url("' + _this.items[_this.right].image + '")';
      ...
      case false:
        ...
        _this.center =
          _this.center > 0 ? _this.center - 1 : _this.items.length - 1;
        _this.right =
          _this.right > 0 ? _this.right - 1 : _this.items.length - 1;

        outView.style.backgroundImage =
          'url("' + _this.items[_this.left].image + '")';
  ...
  _this.populate = function () {
    ...
    _this.eventListeners({
      arrows: [elements.arrows.left, elements.arrows.right],
    });
  };
